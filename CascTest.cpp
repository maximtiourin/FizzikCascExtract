...
int main(int argc, char * argv[])
{
	...
	if (argc == 1) {
		//Heroes
		const TCHAR * szListFile = _T("listfiles\\heroes.listfile");
	}
	else if (argc == 2) {
		//Images
		const TCHAR * szListFile = _T("listfiles\\images.listfile");
	}
	...
	if (nError == ERROR_SUCCESS) {
		nError = TestOpenStorage_ExtractFiles(MAKE_PATH("D:\Program Files (x86)\Heroes of the Storm\HeroesData"), _T("heroesdata"), szListFile);
	}
